...
author: "Jingchao Feng" # Remember to add your name!
date: "16-10-2024"
theme: united
---
...
```{r}
## Import data
cantril<-read.csv(here("data/happiness-cantril-ladder.csv"))%>%
  rename(Country=Entity)
filtered_data<-cantril%>%
  filter(Year%in%c(2011,2023), Country%in%c("China","India","Uganda",	"Switzerland"))
wide_data<-filtered_data%>%
  pivot_wider(names_from = Year,values_from =`Cantril.ladder.score`,names_prefix = "",names_sep = "_" )
...
wide_data<-wide_data%>%
  mutate(
    `Absolute change`=round(`2023`-`2011`,2),`Relative change`=round(((`2023`-2011)/`2011`)*100,2)
  )%>%
  mutate(`Relative change`=scales::percent(`Relative change`))
reactable(wide_data,defaultColDef = colDef(align="center"))
```
...
selected_countries <- c("China","India","Uganda",	"Switzerland") # Replace with actual country names
filtered_countries <- cantril %>%
  filter(Country %in% selected_countries)

line_chart <- ggplot(filtered_countries, aes(x = Year, y = `Cantril.ladder.score`, color = Country)) +
  geom_line() +
...
function filter_default() {
  var default_countries = [""]; // Set default countries here
